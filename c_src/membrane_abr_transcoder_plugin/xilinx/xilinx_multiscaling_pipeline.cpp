#include "xilinx_multiscaling_pipeline.h"
...
std::string createGraphFilterSpec(MultiScalerInput input,
  ...
  int highest_output_framerate =
      std::max_element(outputs.begin(),
                       outputs.end(),
  ...
  if (input.framerate > highest_output_framerate) {
    ss << "[in]fps=" << input.framerate / 2 << "[source]; [source]";
  }

  ss << "multiscale_xma=outputs=" << outputs.size()
     << ": lxlnx_hwdev=" << device_id;

  for (size_t i = 0; i < outputs.size(); i++) {
    auto rate_type =
        outputs[i].framerate < highest_output_framerate ? "half" : "full";
    ss << ": out_" << i + 1 << "_width=" << outputs[i].width << ": "
       << "out_" << i + 1 << "_height=" << outputs[i].height << ": "
       << "out_" << i + 1 << "_rate=" << rate_type;
  }
...
XilinxMultiscalingPipeline::XilinxMultiscalingPipeline(
  ...
  auto graph_filter_spec = createGraphFilterSpec(input, outputs, device_id);
  ...
  ss << "video_size=" << input.width << "x" << input.height
     << ":pix_fmt=" << AV_PIX_FMT_XVBM_8 << ":time_base=" << 1 << "/"
  ...
  if ((ret = avfilter_graph_create_filter(
           &buffer_source, buffersrc, "in", args.c_str(), NULL, graph)) < 0) {
  ...
  for (int i = 0; i < (int)outputs.size(); i++) {
    ...
    if ((ret = avfilter_graph_create_filter(
             &buffer_sinks[i], buffersink, link_name, NULL, NULL, graph)) < 0) {
  ...
  ret = avfilter_graph_parse_ptr(
      graph, graph_filter_spec.c_str(), &filter_inputs, &filter_outputs, NULL);
  if (ret < 0) {
  ...
  auto output_frame_cmp = [](const auto& a, const auto& b) {
    return a.framerate < b.framerate;
  };

  auto max_output =
      std::max_element(outputs.begin(), outputs.end(), output_frame_cmp);
  input_half_rate_filter_applied = max_output->framerate < input.framerate;
  for (const auto& output : outputs) {
    output_half_rate_filter_applied[output.id] =
        output.framerate < max_output->framerate;
  }
  ...
  any_half_rate_filter_applied =
      any_output_halved | input_half_rate_filter_applied;
...
bool XilinxMultiscalingPipeline::RequiresTimestampHalving(
    ScalerOutputID output_id) const {
  auto max_output_cmp = [](const auto& a, const auto& b) {
    return a.framerate < b.framerate;
  };
  auto max_output =
      std::max_element(outputs.begin(), outputs.end(), max_output_cmp);

  return outputs[output_id].framerate < max_output->framerate;
}

bool XilinxMultiscalingPipeline::RequiresOffsetHalving(
    ScalerOutputID output_id) const {
  (void)output_id;
  ...
  return input.framerate > max_output->framerate;
}

void XilinxMultiscalingPipeline::EnqueueKeyFrame(
  ...
  if (video_frame.is_keyframe) {
    for (const auto& output : outputs) {
      ...
      if (output_half_rate_filter_applied[output.id] ||
          input_half_rate_filter_applied) {
...
bool XilinxMultiscalingPipeline::DequeueKeyFrame(ScalerOutputID output_id,
                                                 uint32_t frame_num) {
...
void XilinxMultiscalingPipeline::EnqueueFrameGap(
  ...
  for (const auto& output : outputs) {
    ...
    if (!output_half_rate_filter_applied[output.id] &&
        !input_half_rate_filter_applied && repeat_current_input_frame) {
      EnqueueFrameSkip(output.id, current_frame_number - 1);
    }
  }
}

uint32_t XilinxMultiscalingPipeline::DequeueFrameGap(ScalerOutputID output_id,
                                                     uint32_t frame_num) {
...
void XilinxMultiscalingPipeline::EnqueueFrameSkip(ScalerOutputID output_id,
                                                  uint32_t frame_num) {
  skip_frame_positions[output_id].push(frame_num);
}

bool XilinxMultiscalingPipeline::DequeueFrameSkip(ScalerOutputID output_id,
                                                  uint32_t frame_num) {
...
void XilinxMultiscalingPipeline::Process(VideoFrame<AVFrame>& video_frame) {
...
void XilinxMultiscalingPipeline::Flush() { return; }

std::optional<std::pair<ScalerOutputID, VideoFrame<AVFrame>>>
XilinxMultiscalingPipeline::GetNext() {
  while (true) {
